...
def build_equity_df(account):
    ...
    df['timestamp'] = pd.to_datetime(df['timestamp']) # timestamp를 datetime 객체로 변환하여 정렬에 사용
...
def get_portfolio_status():
    ...
    for acc in ['통합'] + ACCOUNTS:
        ...
        for ticker in acc_all_tickers:
            ...
            # 해당 티커의 모든 거래 내역 (현재 계좌 또는 통합)
            ticker_transactions_df = pd.DataFrame()
            if not df_acc.empty:
                df_t     = df_acc[df_acc['ticker'] == ticker]
                ticker_transactions_df = df_t.sort_values(by='timestamp')

            if ticker_transactions_df.empty and target_w <= 0:
                continue

            # 현재 보유 수량 및 평균 단가 계산 (매도 시 원가 차감 로직 포함)
            current_quantity = 0.0
            current_cost_basis = 0.0

            for _, tx_row in ticker_transactions_df.iterrows():
                tx_type = tx_row['type']
                tx_quantity = tx_row['quantity']
                tx_price = tx_row['price']

                if tx_type == 'BUY':
                    current_cost_basis += (tx_quantity * tx_price)
                    current_quantity += tx_quantity
                elif tx_type == 'SELL':
                    if current_quantity > 0:
                        # 평균 단가법으로 매도 원가 차감
                        avg_cost_per_share_at_sale = current_cost_basis / current_quantity
                        cost_of_sold_shares = tx_quantity * avg_cost_per_share_at_sale
                        current_cost_basis -= cost_of_sold_shares
                        current_quantity -= tx_quantity
                        
                        # 보유 수량이 0 이하가 되면 원가도 0으로 초기화 (포지션 청산)
                        if current_quantity <= 0:
                            current_quantity = 0.0
                            current_cost_basis = 0.0
            ...
            # 보유 수량이 없고 목표 비중도 없다면 표시하지 않음
            if current_quantity <= 0 and target_w <= 0:
                continue
            ...
            grouped.append({
                ...
                '평단가':      round(avg_price), # 수정된 avg_price 사용
                '현재가':      cur_price,
